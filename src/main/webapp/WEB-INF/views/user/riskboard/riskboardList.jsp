...
<script type="text/javascript">
	$(function() {
		...
		$('.btn-write').on('click', function() {
			...
			// 로그인 한 상태!
			location.href = "${pageContext.request.contextPath}/user/riskboard/riskboardForm.do?mem_id=${MEMBER_LOGININFO.mem_id}&project_no=${param.project_no}";
		});
	});
	
	function viewBoardInfo(e) {
		const risk_no = $(e).find('input[name=risk_no]').val();
		const mem_id = $(e).find('input[name=mem_id]').val();
		const project_no = $(e).find('input[name=project_no]').val();
		
		
		location.href = "${pageContext.request.contextPath}/user/riskboard/riskboardView.do?risk_no=" + risk_no + "&mem_id=${MEMBER_LOGININFO.mem_id}" + "&project_no=" + project_no;
    ...
    <div class="card">
      ...
      <div class="card-body">
        <ul class="nav nav-pills nav-fill flex-column flex-sm-row" id="tabs-text" role="tablist">
          <li class="nav-item">
            <button type="button" class="btn btn-primary btn-lg" style="width: 450px;"  onclick="javascript:location.href='${pageContext.request.contextPath}/user/riskboard/riskboardList.do?project_no=${param.project_no }'">위험 관리 게시판</button>
          </li>
          <li class="nav-item">
            <button type="button" class="btn btn-secondary btn-lg" style="width: 450px;"  onclick="javascript:location.href='${pageContext.request.contextPath}/user/riskboard/riskboardList.do?project_no=${param.project_no }'">이슈 게시판</button>
          </li>
          <li class="nav-item">
	...
	<div class="row">
		<div class="col">
			<div class="card">
				<!-- Card header -->
				<div class="card-header">
					<h3 class="mb-0">위험관리 게시판</h3>
					<p class="text-sm mb-0">위험관리 게시판입니다.</p>
				</div>
				<div class="table-responsive py-4">
					<table class="table table-flush datatable-basic">
						...
						<tbody>
							<c:forEach items="${riskboardList }" var="item">
								<tr class="table-row-data" onclick="javascript:viewBoardInfo(this)">
									<td>${item.r }<input type="hidden" name="risk_no" value="${item.risk_no }"></td>
									<td>${item.risk_title }</td>
									<c:if test="${item.risk_errorstatus == '오류 해결' }">
										<td style="color:green;">${item.risk_errorstatus}</td>
									</c:if>
									<c:if test="${item.risk_errorstatus != '오류 해결' }">
										<td style="color:red;">${item.risk_errorstatus}</td>
									</c:if>
									<td>${item.mem_id }<input type="hidden" name="mem_id" value="${item.mem_id }"></td>
									<td>${fn:split(item.risk_regdate,' ')[0]}</td>
									<td>${item.risk_hit }</td>
