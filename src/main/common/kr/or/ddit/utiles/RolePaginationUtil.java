...
public class RolePaginationUtil {
	...
	private int totalPage; // 전체 페이지갯수
	...
	public void RolePaginationUtilMethod(HttpServletRequest request, int currentPage,
			int totalCount) {
	...
	private void makePagination() {
		...
		// 페이지 별 페이지네이션 메뉴 갯수 시작 페이지 번호, 끝 페이지 번호
		// 이전 |1|2|3|4|5|다음
		// 이전 |6|7|8|9|10|다음
		...
		if ((this.currentPage - 1) == 0) {
			...
			this.pagingHtmls.append("<li><a href='" + requestURI
		...
		for (int i = this.startPage; i <= this.endPage; i++) {
			if (this.currentPage == i) {
				this.pagingHtmls.append("<li class='active'><a href='#'>" + i
						+ "</a></li>");
			} else {
				this.pagingHtmls.append("<li><a href='" + requestURI
